...
export class AiService {
  ...
  /**
   * Pre-signup patient assistant chat (the flow's "chat" entry path). Stateless:
   * the full message history comes from the client and nothing is persisted —
   * the late auth gate means there is no user record to attach it to yet.
   * Returns the reply plus structured journey hints (detected case fields,
   * savings estimate, next step) that the frontend renders as cards/CTAs.
   */
  async assistantChat(params: {
    messages: { role: 'user' | 'assistant'; content: string }[];
    language?: string;
  }) {
    // Defensive caps: the endpoint is public, so bound what reaches the model.
    ...
    const mockFallback = () => ({
      reply:
        "I'm having trouble reaching our AI right now, but you don't have to wait: you can browse ranked hospitals with transparent total prices, or upload your medical reports for a free analysis.",
...
Return STRICT JSON: {"date": "YYYY-MM-DD"} — or {"date": null} if the text names no resolvable date.`,
  ...
  /**
   * Translate the frontend UI string catalog into a target language. Powers the
   * "any language" selector: languages without a hand-written locale bundle get
   * an AI-translated one at runtime (cached client-side). Keys pass through
   * unchanged; only values are translated.
   */
  async translateUi(params: { language: string; strings: Record<string, string> }) {
    // Defensive caps — public endpoint: bound entry count and value length.
    const entries = Object.entries(params.strings || {})
      .slice(0, 200)
      .map(([k, v]) => [String(k).slice(0, 120), String(v).slice(0, 700)] as [string, string]);
...
- Never add, drop, or rename keys.`,
    ...
    // Translate in small chunks: non-Latin scripts (Tamil, Amharic, …) burn many
    // more output tokens per word — one big call truncates mid-JSON and the whole
    // catalog is lost. Small chunks always fit; a failed chunk just falls back to
    // English for those keys (the client merges per-key).
    const CHUNK = 15;
    ...
    for (let i = 0; i < entries.length; i += CHUNK) {
      ...
      try {
        const res = await this.chat(
          [system, { role: 'user', content: JSON.stringify(chunk) }],
          2500,
          undefined,
          0.2,
        );
        for (const [k, v] of Object.entries(res || {})) {
          if (typeof v === 'string' && k in params.strings) out[k] = v;
        }
      } catch (err: any) {
  ...
  async analyzeReport(params: {
    fileBase64?: string;
    fileType?: string;
    files?: { base64: string; type: string }[]; // multi-document: several images
    reportText?: string;
    description?: string;
    treatment?: string;
    country?: string;
    ...
    // Attach every uploaded image (multi-document support). Falls back to the
    // legacy single-file fields when `files` isn't provided.
...
- 3-5 flags with at least one warning and one info. Set patientCountry only if explicitly stated.`,
    ...
    if (result && !result.report) {
      ...
      const scoreBySev: Record<string, number> = { Low: 15, Mild: 30, Moderate: 50, 'Moderate-High': 65, High: 75, Severe: 78, Critical: 90 };
      result.report = {
        overview: {
        ...
        correlation: { consistency: 'N/A', treatmentCorrelation: 'N/A', trend: 'N/A', keyFlags: (result.flags || []).map((f: any) => f.text).join('; ') },
        impression: result.diagnosis?.plain || '',
        recommendations: ['Consult a specialist to confirm findings and plan treatment.'],
      };
    }

    // Backfill the "Treatment Information" section from the request context when
    // the model left it blank. Common for description-only analyses: the patient
    // is SEEKING treatment, so there's no administered-treatment data to extract
    // and the model returns all "N/A" — but we still know the sought treatment
    // and their described condition from the journey/chat intake.
    const tr = result?.report?.treatment;
    if (tr) {
      const blank = (v: any) => !v || !String(v).trim() || String(v).trim().toUpperCase() === 'N/A';
      if (blank(tr.type) && params.treatment) {
        tr.type = params.treatment.replace(/[-_]/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase());
      }
      if (blank(tr.description) && params.description) tr.description = params.description;
    }
    return result;
  }
...
Rules: base everything strictly on the transcript. "agreedQuoteUsd" is the final all-inclusive surgery/package price the HOSPITAL quoted, in USD (a single number); if no price was quoted, set it to null. Do not invent numbers.`,
  ...
  /**
   * Localize one review: detect its language FROM THE TEXT, translate to English, and
   * reconstruct the native script (legacy scrapes store non-English text romanized).
   * Returns { lang, isEnglish, english, native }. One call does all three.
   */
  async localizeReview(text: string): Promise<{ lang: string; isEnglish: boolean; english: string; native: string }> {
    return this.chat(
      [
        {
          role: 'system',
...
Respond ONLY as JSON: {"lang":"ISO code","isEnglish":true|false,"english":"...","native":"..."}.`,
  ...
  /**
   * Classify a batch of candidate videos as medical-tourism leads. Used by the
   * lead-generation pipeline AFTER a cheap keyword pre-filter, so only promising
   * candidates reach the model. Batched (many per call) to keep AI cost low.
   * Returns one verdict per input, keyed by the candidate's `id`.
   */
  async classifyLeads(
    candidates: { id: string; title: string; description?: string }[],
  ): Promise<Record<string, { isLead: boolean; confidence: number; persona: string | null; alreadyTreated: boolean; procedure: string | null; summary: string }>> {
...
Respond ONLY as JSON: {"results":[{"id":"<id>","isLead":true,"confidence":90,"persona":"researching","alreadyTreated":false,"procedure":"knee replacement","summary":"..."}]}`,
  ...
  /**
   * Classify a batch of captured social posts (Reddit/Quora/Instagram/Facebook/X)
   * by their BODY text into one of four categories for the lead-analytics dashboard:
   *   LEAD      — a real individual (patient/family) personally seeking or discussing
   *               treatment abroad for their OWN situation (asking for help/advice/
   *               experiences/cost, sharing their own journey).
   *   MARKETING — promotional / advertising / self-promotion / spam: agencies,
   *               hospitals, clinics, facilitators, influencers or anyone selling or
   *               promoting services, packages, discounts, "DM us", service links.
   *   NEWS      — news articles, journalistic/informational/educational broadcast,
   *               reports, statistics, general non-personal discussion.
   *   OTHER     — off-topic, irrelevant, unintelligible, or not medical-tourism related.
   * Batched (many posts per call) to keep AI cost low. Returns one verdict per input,
   * keyed by the post's `id`.
   */
  async classifyCategories(
    items: { id: string; platform?: string; title?: string | null; body?: string | null }[],
    examples?: { text: string; category: string; reason?: string }[],
    temperature?: number,
  ): Promise<Record<string, { category: 'LEAD' | 'PARTNER' | 'MARKETING' | 'NEWS' | 'OTHER'; reason: string; confidence: number }>> {
    ...
    const list = items
      ...
      .join('\n');
    // Few-shot exemplars, sourced from our own labelled DB rows (LEAD-weighted). Empty ⇒ zero-shot.
...
Respond ONLY as JSON: {"results":[{"id":"<id>","category":"LEAD","reason":"patient asking for knee surgery cost in India","confidence":90}]}`,
    ...
    const map: Record<string, { category: 'LEAD' | 'PARTNER' | 'MARKETING' | 'NEWS' | 'OTHER'; reason: string; confidence: number }> = {};
    for (const r of result?.results || []) {
      const cat = String(r?.category || '').toUpperCase();
      if (r?.id && valid.has(cat)) {
        const conf = Math.max(0, Math.min(100, Number(r.confidence)));
        map[r.id] = { category: cat as any, reason: String(r.reason || '').slice(0, 200), confidence: Number.isFinite(conf) ? conf : 60 };
      }
    }
    return map;
  }
  ...
  /**
   * Analyze ONE YouTube video's full TRANSCRIPT to qualify and score it as a
   * medical-tourism lead. Reads the actual spoken content (not just the title), so
   * it's far more accurate than title/description classification. One call per video.
   */
  async analyzeTranscript(input: {
    ...
    isLead: boolean; score: number; confidence: number; persona: string | null;
    alreadyTreated: boolean; procedure: string | null; originCountry: string | null; summary: string;
  }> {
    const result = await this.chat(
      [
        {
...
isLead = true only when score >= 55. Return JSON ONLY: {"isLead":bool,"score":0-100,"confidence":0-100,"persona":"undecided"|"cant_afford"|"suffering"|"researching"|"journey"|null,"alreadyTreated":bool,"procedure":"condition/treatment or null","originCountry":"speaker's home country if stated, else null","summary":"one short sentence on who they are and what they need"}`,
  ...
  /**
   * Enrich a single hospital's comparison-card data from its name/city/rating and
   * a sample of its real reviews. Used by the scrape pipeline (new hospitals) and the
   * admin enrichment endpoint. Returns the structured profile (caller persists it).
   */
