...
export class TripPlanService {
  ...
  async generate(params: { hospitalId: string; diagnosis: string; treatment: string; country: string }) {
    ...
    // AI fallback for unknown procedure+destination
    ...
    try {
      await this.prisma.tripPlanTemplate.create({
        data: {
          ...
          timeline: (tripPlan as any).timeline ?? [],
          costs: (tripPlan as any).costs ?? {},
          totalEstimate: (tripPlan as any).totalEstimate ?? '',
          travelTips: (tripPlan as any).travelTips ?? [],
        },
      });
    } catch { /* ignore */ }
